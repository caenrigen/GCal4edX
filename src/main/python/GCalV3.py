...
class GCalV3(object):
    ...
    def createGCal(self):
        calBody = {"summary": self.targetCalName, "timeZone": self.timeZone}
    ...
    def getAllEvents(self):
        ...
        while True:
            events = (
                self.service.events()
                .list(calendarId=self.getCalId(), pageToken=page_token)
                .execute()
            )
    ...
    def clearEventsInCal(self, allEvents=None):
        ...
        for event in allEvents:
            self.service.events().delete(
                calendarId=self.getCalId(), eventId=event["id"]
            ).execute()
    ...
    def getCalPublicLink(self):
        ...
        return (
            publicLinkPrefix
            + urllib.parse.quote_plus(self.calendar["id"])
            + publicLinkSufix
        )
    ...
    def uploadEvents(self, events):
        for event in events:
            self.service.events().insert(
                calendarId=self.getCalId(), body=event
            ).execute()
        ...
        for cal in calendars:
            if cal["summary"] == self.targetCalName:
                if not self.calendar:
                    ...
                    print(
                        "Found one more calendar with same name! Only the first was cleared!"
                    )
